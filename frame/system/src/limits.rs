...
impl BlockWeights {
	...
	pub fn validate(self) -> ValidationResult {
		...
		for class in DispatchClass::all() {
			...
			log::error!("validate 444, [{:?} {:?}#{:?} (total) has to be greater than {:?}#{:?} (base block) & {:?}#{:?} (base extrinsic)", class, max_for_class.ref_time(), max_for_class.proof_size(), self.base_block.ref_time(), self.base_block.proof_size(), base_for_class.ref_time(), base_for_class.proof_size());
